...
class RLVRConfig(PPOConfig):
    ...
    dynamic_prompt_logp_loss_weight: bool = field(
        default=False,
        metadata={
            "help": (
                "Use online prompt likelihood weighting. Before each optimizer update, the current actor computes "
                "each response sample's prompt NLL and scales its policy loss by the data-parallel optimizer-batch "
                "mean NLL divided by the sample NLL."
            )
        },
    )
